...
```
veyl init myapp
veyl add github.com/someone/veyl-strutil@v1.0.0
```
...
called something like `veyl-strutil`, which cannot be a name in code -
`veyl-strutil.titleCase` reads as a subtraction. `veyl add` says so
and tells you to pick one:

```
veyl add github.com/someone/veyl-strutil@v1.0.0 as strutil
```

**A bare name is a package; a name ending in `.vy` is a file.** That is
the whole rule, and it is why the two can never be confused:

```qz
import "helpers.vy"    // a file next to this one
import "strutil"       // a package from veyl.json
```
...
| Command | Effect |
| --- | --- |
| `veyl init [name]` | create `veyl.json` here |
| `veyl add <source>` | fetch a package and record it |
| `veyl add <source> as <name>` | ...under a different import name |
| `veyl remove <name>` | drop a dependency |
| `veyl install` | fetch everything the manifest lists |
| `veyl packages` | list dependencies and whether they are installed |

You rarely need `veyl install` yourself - a missing package is fetched
...
```json
{
  ...
  "version": "1.0.0",
  "main": "strutil.vy",
  "description": "String helpers Veyl does not ship with",
  "dependencies": {
...
| `main` | The file an importer gets. Defaults to `<name>.vy`. |
...
moves, and while `veyl.lock` will notice, being told your dependency
...
```json
{
  "packages": {
    "strutil": {
      "source": "github.com/someone/veyl-strutil@v1.0.0",
      "version": "v1.0.0",
      "sha256": "9f2c1a..."
    }
  }
...
different code. `veyl install` recomputes the hash and stops if it
does not match:

```
strutil: the contents of github.com/someone/veyl-strutil@v1.0.0 changed
    since it was locked.
    expected 9f2c1a0b3d4e
    got      771be3c8a012
    The tag was moved. Check what changed before trusting it;
    'veyl add ...' again if the new code is what you want.
```

**Commit `veyl.lock`.** It is what makes a checkout on another machine
...
A package is a directory with a `veyl.json` and at least one `.vy`
...
```
veyl add github.com/you/veyl-strutil@v1.0.0
```
...
they edit the version in `veyl.json` and not before, which is the
...
- `veyl.json` has `name`, `version`, `main`, `description`
...
- It compiles from a clean checkout: `veyl run` something that uses it
...
```
veyl add ../veyl-strutil
```

```json
"dependencies": {
  "strutil": "../veyl-strutil"
}
```
...
Local dependencies are deliberately **not** written to `veyl.lock` -
...
```
%LOCALAPPDATA%\veyl\pkg\github.com\owner\repo@v1.0.0\
```
...
`veyl.json` at or above the file doing the importing, resolves it to
...
A relative `import "helpers.vy"` is **not** namespaced. Files in the
...
depend on. The cost: no search, no discovery, no `veyl search json`.
...
**Trust on first use.** The first `veyl add` records whatever it
...
**No `veyl search`, no discovery.** Consequence of having no
registry.

...
// Veyl compiles to Go source and then shells out to the Go toolchain,
...
//
// That names the wrong problem. Someone who installed Veyl has no
...
// veyl.exe. It is deliberately kept off PATH: a developer who already
...
//
//  1. $VEYL_GO, for anyone who wants to force a specific one
//  2. a private toolchain shipped beside veyl.exe by the installer
//  3. PATH, which is the normal case on a development machine
//
// The bundled copy beats PATH so that an installed Veyl keeps working
...
func findGo() (goToolchain, error) {
	...
	if forced := os.Getenv("VEYL_GO"); forced != "" {
		if isExecutable(forced) {
			return goToolchain{forced, "VEYL_GO"}, nil
		}
		return goToolchain{}, fmt.Errorf("VEYL_GO is set to %s, but there is no runnable Go compiler there", forced)
	}

	if dir, err := os.Executable(); err == nil {
		// Resolve symlinks so a linked veyl still finds its own files.
		if real, err := filepath.EvalSymlinks(dir); err == nil {
			dir = real
		}
		bundled := filepath.Join(filepath.Dir(dir), "go", "bin", name)
		if isExecutable(bundled) {
			return goToolchain{bundled, "the copy installed with Veyl"}, nil
		}
	}
...
// errNoGo explains the problem in terms of Veyl rather than PATH, and
// says what to do about it.
func errNoGo() error {
	return fmt.Errorf(`Veyl needs the Go toolchain to build programs, and could not find one.

Looked for it in three places:
  - the VEYL_GO environment variable            (not set)
  - a copy installed alongside veyl.exe         (not there)
  - PATH                                          (no 'go' on it)

Either install Go from https://go.dev/dl, or reinstall Veyl and leave
the bundled toolchain option ticked. Run 'veyl doctor' afterwards to
...
// doctor reports what Veyl can see, so that "it doesn't work" on
...
func doctor() error {
	...
	fmt.Println("\nEverything Veyl needs is present.")
	return nil
}
